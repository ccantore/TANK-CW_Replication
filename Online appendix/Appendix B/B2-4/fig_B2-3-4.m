...
%% LF manually forced response of bonds in UH model for H to 0!
...
% Output options
OptionPrint = 1;        
FigName = 'fig_sensi_UH_lambda_linear';
TargetPath = './Output/';


% Names of files to be loaded (.mat) whether they're linear or non-linear, and
% how they're called in figures
RESULT_names={'tank_uh_results_lambda01','tank_uh_results_lambda05'};%,'TANKs_results'}; %%EXACT NAMES OF THE RESULTS FILES TO LOAD
%RESULT_names={'tank_uw_results_lambda01','tank_uw_results_lambda05','tank_uw_results_lambda09'};%,'TANKs_results'}; %%EXACT NAMES OF THE RESULTS FILES TO LOAD
%RESULT_names={'tank_cw_results_lambda01','tank_cw_results_lambda05','tank_cw_results_lambda09'};%,'TANKs_results'}; %%EXACT NAMES OF THE RESULTS FILES TO LOAD

lin_vs_nonlin=[1,1,1]; % = 0 if corresponding model non-linear, 1 otherwise
Model_names=char('\lambda=0.1','\lambda=0.5'); %,'\lambda=0.9'); %NAMES OF THE MODEL VARIANTS TO APPEAR IN THE LEGEND OF THE GRAPHS
...
%VAR_IRFs_linear = {'Y','H','C','CSl','CHl','LI','LS','BS','BH','profits','W','R'};%,'RR','RnRn','PIEPIE','tauC','tauK','tauW','BG','BGC','BGW',}; %% NAMES OF ENDOGENOUS VARIABLES OF INTEREST FOR THE IRFS (NON-LINEAR MODELS) - NEED TO HAVE EXACTLY SAME NAME AS IN THE .MOD FILE
VAR_IRFs_linear = {'n','n','w','c','csl','chl','b','bsl','bhl','t','ls','d'};   
names = char('Output','Hours worked','Real wage','Consumption','Consumption U','Consumption H','Bonds','Bonds U','Bonds H', 'Taxes','Labor share','Profits');
NAME_SHOCKS={'_eps'};
shocks_names=['T'; 'M'; 'F'];
...
%vColors = {[0.33 0.33 0.33],[0.2,0.2,0.2],[0.46,0.46,0.46],[0.6,0.6,0.6]}; %UW
%vColors = {[0.23,0.23,0.63],[0,0,90]/255,[0.45,0.45,0.75], [0.68, 0.68,0.88]}; %CW
vColors = {[255,170,60]/255,[255 127 0]/255,[255,190,133]/255,[254,237,222]/255}; %UH
%{,,,};
%{,,,[114,47,55]/255};
else
...
if abs(irfs_matrix(1:IRFPeriods,jj,xx,mm))<10^(-5)
 plot(0*irfs_matrix(1:IRFPeriods,jj,xx,mm),vLinestyle{mm},'Color',vColors{mm},'LineWidth',LinewidthDefault); 
else
plot(irfs_matrix(1:IRFPeriods,jj,xx,mm),vLinestyle{mm},'Color',vColors{mm},'LineWidth',LinewidthDefault); 
end
hold on;
